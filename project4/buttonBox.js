...
// server code
    ...
    case '/':
        ...
        res.write('<h1>Project 04!</h1>Try<ul><li><a href="/buttonBox.html">This is Project 04. Modified by Li Xia.</a></li></ul>');
...
//            console.log('emitted gpio: ' + data);
    ...
    */
    ...
    function push() {
	var ainPath = "/sys/devices/platform/omap/tsc/ain6";
        fs.readFile(ainPath, 'base64', function(err, data) {
            if(err) throw err;
            socket.emit('ain', data);
	});
	    
	var gpioPath = "/sys/class/gpio/gpio7/value";
        fs.readFile(gpioPath, 'base64', function(err, data) {
            if(err) throw err;
            socket.emit('gpio', data);
	});
